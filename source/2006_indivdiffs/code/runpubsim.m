...
% data parameters
load publicationdata;
X=pubdata; % data
n=size(X,1); % number of people
s=sum(X,2); % obs per person
M=size(X,2); % number of response options

% simulation parameters
fname='dppubsim'; % filename
...
% save data set
eval(['save ' fname ' X* n s M burnin thinning nsamp']);

% housekeeping
AA=sparse(nsamp,1); % empty matrix of alphas
KK=sparse(nsamp,1); % empty matrix of k
GG=sparse(nsamp,n); % empty matrix of assignments
MB=M*B; % summed betas
...
NN=n; % number of people assigned to each group
...
% load recorded data as needed
load tempstate

while ss<=nsamp + burnin/thinning;
    ...
    for i=1:thinning; 
        ...
        for p=1:n % over all people
            ...
            NN(g)=NN(g)-1; % decrease the count of people in the group
            ...
            if NN(g)==0
                ...
                NN=NN(tmp); % delete from group size vector
    ...
    if ~burntin; 
        ...
        %%eval(['save ' fname ' AA KK GG -append']);
